...
def add_edges(status, targetX, edge, trapezoid_decomposition):
    ...
    try:
        ...
        None
    if upper != None and ((upper[1].is_left_to_right() and upper[1].insideOn == Direction.Right) or
                              (upper[1].is_right_to_left() and upper[1].insideOn == Direction.Left)):
    ...
    if lower != None and ((lower[1].is_right_to_left() and lower[1].insideOn == Direction.Right) or
                              (lower[1].is_left_to_right() and lower[1].insideOn == Direction.Left)):
        i = "({}, {}),".format(edge.point_at_edge(targetX).x, edge.point_at_edge(targetX).y)
...
def trapezoid_decompose(edges):
    ...
    while len(event_queue) > 1:
        ...
        if not draw_td_edge:
            ...
            draw_td_edge = True
        print("Status size is {}".format(len(status)))
